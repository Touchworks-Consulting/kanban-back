...
async function testFeedbackVotingProduction() {
  ...
  try {
    ...
    if (feedbacksWithVotes && feedbacksWithVotes.length > 0) {
    ...
    console.log('🔗 Foreign keys encontradas:', Array.isArray(foreignKeys) ? foreignKeys.length : 0);
    ...
    console.log(`   ✅ Índices únicos: ${Array.isArray(indexes) ? indexes.length : 0} encontrados`);
    console.log(`   ✅ Foreign keys: ${Array.isArray(foreignKeys) ? foreignKeys.length : 0} configuradas`);
